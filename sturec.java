...
//Table
	...
	sturec() {
		...
		add(ladm);
		/*
		 * top=new JPanel(); top.setLayout(null);
		 * top.setBorder(BorderFactory.createEtchedBorder(1));
		 * top.setBounds(60,20,320,50); add(top);
		 */
		...
		/*
		 * ldate=new JLabel("Date :"); ldate.setBounds(270,100,35,30); tdate=new
		 * JTextField(); tdate.setBounds(320,100,140,25);
		 */
...
// Table
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == bcle) {
			...
			/*
			 * tsname.setEnabled(false); tsadd.setEnabled(false); tspno.setEnabled(false);
			 * tbat.setEnabled(false); tgen.setEnabled(false); tcou.setEnabled(false);
			 * //tdate.setEnabled(false);
			 */
		...
		if (ae.getSource() == bcancel) {
			...
			try {
				...
				while (rs.next()) {
					model.insertRow(r++, new Object[] { rs.getString(3), rs.getString(4), rs.getString(5),
		...
		if (ae.getSource() == bdel) {
			...
			try {
				...
				/*
				 * trno.setEnabled(true); tsname.setEnabled(false); tsadd.setEnabled(false);
				 * tspno.setEnabled(false); tbat.setEnabled(false); tgen.setEnabled(false);
				 * tcou.setEnabled(false); //tdate.setEnabled(false);
				 */
		...
		if (ae.getSource() == bopen) {
			try {
				...
				while (rs.next()) {
					...
					/*
					 * tsname.setEnabled(true); tsadd.setEnabled(true); tspno.setEnabled(true);
					 * tbat.setEnabled(true); tgen.setEnabled(true); tcou.setEnabled(true);
